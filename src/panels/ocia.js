  ...
  if(exp) {
    ...
    if(docs.length) {
      ...
      h += docs.map((d,i) => `<div class="doc-item">
        ...
        <span style="color:${d.done?'#2D6A4F':'#922B21'};cursor:pointer;" onclick="toggleOciaDoc('${person.id}',${i})">${d.name}</span>
        <button class="doc-del-btn" onclick="deleteOciaDoc('${person.id}',${i})">✕</button>
      </div>`).join('');
    }
